...
void InsertSort(int arr[], int n) //直接插入排序
{
	...
	{
		for (j = i; j > 0;
		     j--) //将a[i]插入到a[i-1]、a[i-2]、a[i-3]...之中
			if (arr[j - 1] >
			    arr[j]) //如果前面的元素比后面大，就交换，另外使用j-1刚好可以处理到arr[0]
				swap(&arr[j - 1], &arr[j]);
	}
}
...
int partition(int arr[], int low, int high) //low,high作为扫描数组的标记
{
	...
	while (true) {
		while (arr[i++] <=
		       key) { //从左向右扫描，直到遇到大于key的元素或者到达右边界
			if (i == high)
				break;
		}
		while (arr[j--] >=
		       key) { //从右向左扫描，直到遇到小于key的元素或者到达左边界
			if (j == low)
				break;
		}
	...
	swap(&arr[low],
	     &arr[j]); //将存放在arr[low]的切分元素放置到他的最终位置j
	return j; //返回j
}
